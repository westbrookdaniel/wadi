...
Use your normal electron-builder signing credentials for Windows signing and macOS signing/notarization. Manual runs of `.github/workflows/desktop-release.yml` upload test artifacts. Matching version tags build all platforms and upload installers and update metadata to a draft GitHub release. The workflow does not deploy the website or publish the draft automatically. The release workflow requires Developer ID signing and notarization on macOS. Configure CSC_LINK, CSC_KEY_PASSWORD, APPLE_ID, APPLE_APP_SPECIFIC_PASSWORD and APPLE_TEAM_ID in GitHub Actions secrets. Windows remains unsigned unless its signing secrets are provided. Redistributing FFmpeg requires including its license/build notices; the preparation script copies notices downloaded with ffmpeg-static. Complete your distribution review before a public release.
...
Before publishing the draft, test an installer and an upgrade from an older installed release, including session retention, pause/resume, download failure, and restart confirmation. Publish the complete draft with its installers, ZIP payload, `latest*.yml` and blockmaps together. The public GitHub release is the update source. The public westbrookdaniel/wadi repository hosts releases. WADI_RELEASE_REPOSITORY and GH_RELEASE_TOKEN can be configured for a separate public release repository. Build metadata embeds the repository, never its token. Tagged builds refuse a private update source. Current installers are unsigned by choice. macOS users may need to approve the app in Privacy & Security; macOS automatic updates require signing, so use the GitHub installer for upgrades until signing is configured. Windows may show a SmartScreen warning. Installer upgrades still need manual verification. See the [electron-builder v26 updater documentation](https://www.electron.build/v26/docs/features/auto-update/).
