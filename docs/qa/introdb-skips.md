...
- All 13 API tests pass against disposable local PostgreSQL 18, including separate user/session/profile preference persistence, default-off behavior, invalid writes and account export.
...
- Run `pnpm db:migrate` to add the default-false `users.introdb_enabled` column before deploying. The hosted API needs this route deployed before a desktop build can obtain timestamps. The installed Wadi app has not been replaced by this PR.
