...
export function DeviceSettings() {
  ...
  return <section className="settings-panel grid gap-5" aria-label="This device">
    ...
    {desktop && !device.tvMode ? <FullscreenStartupSetting desktop={desktop} /> : null}
...
export function ExperimentalSettings() {
  ...
  return <details className="settings-panel group !p-0 overflow-hidden">
    ...
    <label className="flex items-start justify-between gap-6"><span><span className="block text-sm font-medium">TV mode</span><span className="mt-1 block text-sm text-muted-foreground">Large layouts, in-app keyboard and pickers. Arrows to move, Enter / A to choose, Escape / B to go back.</span></span><input className="mt-1 size-5 shrink-0 accent-primary" type="checkbox" aria-label="TV mode" checked={device.tvMode} onChange={event => device.setTvMode(event.target.checked)} /></label>
    </div>
  </details>
}
