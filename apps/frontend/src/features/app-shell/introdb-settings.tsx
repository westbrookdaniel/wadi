...
export function IntroDbSettings() {
  ...
  return <section className="grid gap-5 rounded-xl border border-border bg-card/60 p-5">
    ...
    <label className="flex items-center justify-between gap-5 text-sm">
      ...
      <input type="checkbox" aria-label="Show skip buttons" aria-describedby="introdb-help" className="size-5 shrink-0 accent-primary" checked={!unavailable && preferences.data?.enabled === true} disabled={unavailable || save.isPending} onChange={event => save.mutate(event.target.checked)} />
    </label>
    <p className="text-xs leading-relaxed text-muted-foreground">Powered by <a className="underline underline-offset-4" href="https://introdb.app" target="_blank" rel="noreferrer">IntroDB</a>. Available for supported titles in Wadi’s player. On by default.</p>
