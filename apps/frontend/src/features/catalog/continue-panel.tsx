...
export function ContinuePanel({ items, onOpen }: { items: ContinueWatchingItem[]; onOpen: (media: MediaPreview, videoId?: string | null) => void }) {
  return <section className="continue-panel" aria-label="Continue Watching">
    <div><p className="mb-2 text-xs font-medium tracking-widest text-muted-foreground uppercase">Your evening</p><h2 className="text-2xl font-medium tracking-tight">Continue Watching</h2></div>
    <div className="mt-6 grid content-start gap-2 overflow-y-auto pr-1">
...
function ContinueItem({ item, onOpen }: { item: ContinueWatchingItem; onOpen: (media: MediaPreview, videoId?: string | null) => void }) {
  ...
  return <button type="button" disabled={!media} onClick={() => media && onOpen(media, item.video_id)} className="group flex min-w-0 items-center gap-4 rounded-xl p-2 text-left transition hover:bg-white/5 focus-visible:outline-2 focus-visible:outline-primary" aria-label={media ? `Continue ${media.name}` : 'Loading title'}>
    ...
    <Play aria-hidden="true" className="mr-2 size-4 shrink-0 text-white/65 group-hover:text-white" />
  </button>
}
