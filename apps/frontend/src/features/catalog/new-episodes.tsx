...
export function NewEpisodesSection({ onOpenMedia }: { onOpenMedia: (media: MediaPreview, videoId?: string | null) => void }) {
  ...
  return <section className={contentSection}>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {visible.slice(0, expanded ? visible.length : 6).map(({ show, episode }) => <button key={`${show.media_id}:${episode.id}`} className="group flex min-w-0 items-center gap-4 rounded-xl border border-border bg-card/60 p-3 text-left hover:bg-muted focus-visible:outline-2 focus-visible:outline-ring" onClick={() => onOpenMedia({ id: show.media_id, type: 'series', name: show.title, poster: show.poster ?? undefined, raw: { id: show.media_id, type: 'series' } }, showCalendar ? null : episode.id)}>
