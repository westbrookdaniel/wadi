...
import { ReleaseContext } from '../catalog/release-feed'
...
export function MediaCard({
  ...
  const { counts } = useContext(ReleaseContext)
  const count = counts['name' in media ? media.id : media.media_id] ?? 0
  ...
  return (
    <article className="media-card-item grid min-w-0 gap-2.5 max-[800px]:gap-1.5">
      ...
      >
        ...
        {type === 'Series' && count > 0 && <span aria-label={`${count} new episode${count === 1 ? '' : 's'}`} className="absolute top-2 left-2 rounded-full bg-zinc-800 px-2.5 py-1 text-xs font-semibold text-zinc-100 shadow">+{count}</span>}
        {watched ? (
