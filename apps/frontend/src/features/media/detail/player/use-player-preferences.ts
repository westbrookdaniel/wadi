...
export function usePlayerPreferences({
  ...
  const mergedPrefs = playerDefaults.data
    ? ({ ...playerDefaults.data, ...(playerOverride.data ?? {}) } as HydratedPreferenceSource)
    : null
  ...
  const persistOverride = useCallback(
    (payload: PlayerOverride) => overrideMutation.mutate(payload),
