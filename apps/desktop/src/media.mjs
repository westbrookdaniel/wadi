...
export async function createMediaService({ directory, binaries }) {
  ...
  const start = async payload => {
    ...
    const job={id,path,touched:Date.now(),error:null,process:null};jobs.set(id,job);
    ...
    const launch = chosen => {
      const args=['-hide_banner','-loglevel','error','-nostdin','-y',...networkArgs(input),'-readrate',String(input.speed),'-ss',String(input.position),'-i',input.url];
      ...
      args.push('-sn','-dn','-max_muxing_queue_size','2048','-f','hls','-hls_time','4','-hls_list_size','24','-hls_delete_threshold','4','-hls_flags','delete_segments+temp_file','-hls_segment_filename',join(path,'segment%d.ts'),join(path,'index.m3u8'));
