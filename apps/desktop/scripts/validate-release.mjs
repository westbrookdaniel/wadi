...
if (release) {
  ...
  if (process.env.WADI_UNSIGNED_RELEASE !== '1') for (const name of required) if (!process.env[name]) throw new Error(`Missing signing/notarization setting: ${name}. Windows uses WIN_CSC_LINK and WIN_CSC_KEY_PASSWORD secrets.`);
} else if (!process.env.CSC_LINK && process.platform !== 'linux') {
  console.warn('No signing certificate: installers will be unsigned.');
}
