...
pub async fn migrate(pool: &SqlitePool) -> sqlx::Result<()> {
    ...
    .await;
    let _ = sqlx::query(
        "ALTER TABLE user_settings ADD COLUMN playback_prefs_json TEXT NOT NULL DEFAULT '{}'",
    )
    .execute(pool)
    .await;
    ...
    let user_settings_needs_rebuild = user_settings_schema
        ...
        .map(|sql| {
            !sql.contains("PRIMARY KEY (user_id, profile_id)")
                || !sql.contains("playback_prefs_json")
        })
        .unwrap_or(false);
    ...
    if lists_needs_rebuild || user_settings_needs_rebuild {
        ...
        if user_settings_needs_rebuild {
            ...
            sqlx::query(
                r#"
                INSERT OR REPLACE INTO user_settings_v2 (user_id, profile_id, browse_layout_json, playback_prefs_json, created_at, updated_at)
                SELECT user_id, profile_id, browse_layout_json, COALESCE(playback_prefs_json, '{}'), created_at, updated_at FROM user_settings
                "#,
            )
            .execute(&mut *conn)
            .await?;
