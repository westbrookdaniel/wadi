...
use crate::{
    ...
    models::{
        ...
        ListItem, PlaybackPreferences, Profile, User, UserList, WatchData, WatchState,
    },
...
pub fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            ...
            get(get_playback_preferences).put(set_playback_preferences),
        )
        .with_state(state)
}
...
#[derive(Debug, Deserialize, Default)]
struct PlaybackPreferencesRequest {
    stream_action: Option<String>,
    external_player_template: Option<String>,
}
...
async fn get_playback_preferences(
    State(state): State<AppState>,
    user: AuthUser,
) -> AppResult<Json<PlaybackPreferences>> {
    Ok(Json(
        load_playback_preferences(&state, &user.id, &user.profile_id).await?,
    ))
}

async fn set_playback_preferences(
    State(state): State<AppState>,
    user: AuthUser,
    Json(payload): Json<PlaybackPreferencesRequest>,
) -> AppResult<Json<PlaybackPreferences>> {
    let preferences = normalize_playback_preferences(payload)?;
    let settings_json = serde_json::to_string(&preferences)?;
    sqlx::query(
        r#"
        INSERT INTO user_settings (user_id, profile_id, playback_prefs_json)
        VALUES (?1, ?2, ?3)
        ON CONFLICT(user_id, profile_id) DO UPDATE SET
            playback_prefs_json = excluded.playback_prefs_json,
            updated_at = datetime('now')
        "#,
    )
    .bind(&user.id)
...
async fn load_playback_preferences(
    ...
    user_id: &str,
    profile_id: &str,
) -> AppResult<PlaybackPreferences> {
    let row = sqlx::query(
        "SELECT playback_prefs_json FROM user_settings WHERE user_id = ?1 AND profile_id = ?2",
    )
    .bind(user_id)
    .bind(profile_id)
    .fetch_optional(&state.db)
    ...
    let raw: String = row.try_get("playback_prefs_json")?;
    let parsed = serde_json::from_str::<PlaybackPreferences>(&raw).unwrap_or_default();
    Ok(normalize_playback_preferences_model(parsed))
}

fn normalize_playback_preferences(
    payload: PlaybackPreferencesRequest,
) -> AppResult<PlaybackPreferences> {
    let stream_action = match payload.stream_action.as_deref().map(str::trim) {
        None | Some("") | Some("copy") => "copy",
        Some("external") => "external",
        Some(_) => {
            return Err(AppError::BadRequest(
                "stream_action must be external or copy".into(),
            ))
        }
    };
    let external_player_template = payload
        .external_player_template
        .unwrap_or_else(|| "vlc://{url}".to_string())
        .trim()
        .to_string();

    Ok(normalize_playback_preferences_model(PlaybackPreferences {
        stream_action: stream_action.to_string(),
        external_player_template,
    }))
}

fn normalize_playback_preferences_model(mut value: PlaybackPreferences) -> PlaybackPreferences {
    if value.stream_action != "copy" {
        value.stream_action = "external".to_string();
    }
    value.external_player_template = value.external_player_template.trim().to_string();
    value
}
